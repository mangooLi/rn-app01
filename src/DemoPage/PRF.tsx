...
interface State{
    currentDistance:number,
            ...
            pullRefreshStatus:number,

            showPullStatus:number,

            showPullLastTime:string,
}
...
class PRF extends Component<Props>{
    ...
    //要求成为响应者
    _handleStartShouldSetPanResponder(e: Object, gestureState: PanResponderGestureState): boolean {
        return true;
    }
    _handleMoveShouldSetPanResponder(e: Object, gestureState: PanResponderGestureState): boolean {
        return true;
    }
    //touch down 开始手势操作。给用户一些视觉反馈，让他们知道发生了什么事情！
    _handlePanResponderGrant(e: Object, gestureState: PanResponderGestureState){

    }

    //touch move 响应滑动事件
    _handlePanResponderMove(e: Object, gestureState: PanResponderGestureState) {
    ...
    addZeroAtFront(count:number|string){
        if (count<10){
            count="0"+count;
        }
        return count;
    ...
    render(){
        ...
        if (this.state.showPullStatus===ShowLoadingStatus.SHOW_DOWN){
            indicatorView=<Image
                ...
                source={require('../assets/img/like_red.png')}
